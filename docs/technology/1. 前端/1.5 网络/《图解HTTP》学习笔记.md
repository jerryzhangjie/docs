...
公钥和私钥配对使用。公钥可发给任何人，用以加密信息；私钥不能给任何人，用以解密信息。
...
对称加密性能高，但难点在于如何将密钥安全地告知对方，而非对称加密安全性高，但可以实现安全告知密钥。所以，https结合了二者的优点，即：

   * 非对称加密：使用公钥对密钥进行加密，对方收到加密后的密钥后，使用私钥对其进行解密，得到用于对称加密的密钥。
